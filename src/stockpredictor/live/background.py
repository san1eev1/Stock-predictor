...
class BackgroundTrainer(threading.Thread):
    ...
    def _tune(self, conn) -> None:
        try:
            ...
            self.rounds += 1
            if it is None:
                log.info("Background training: not enough intraday history to tune yet")
                return
            log.info("Background training round %d: intraday IC %.4f (current %.4f)%s",
                     self.rounds, it["ic"], it["previous_ic"],
    ...
    def _learn_live(self, conn, day: date) -> None:
        ...
        try:
            ...
            with MI.MODEL_LOCK:
                model = T.retrain_intraday(self._ctx(), self.store_dir, conn, force=True)
            if model is not None:
                self.model_changed.set()
                log.info("Live learning: intraday model retrained with today's session "
                         "(%d days, up to %s)", model.train_days, model.train_to)
        except Exception:
