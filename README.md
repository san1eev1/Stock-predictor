...
| **Continuous training** | Models **keep training on historical data in the background**: GitHub retrains + self-tunes the long-term and news models on all history since 2005 **twice after each close** (16:45 and 21:00 IST). **The Mac only predicts during the day** (keeps it cool): live prices, picks and paper trading, plus one short intraday retrain after the close; everything heavy runs on GitHub (`MAC_TRAINING=1` in `.env` turns Mac training back on). |
...
| **Mac, after the close** | Live prices stop at 15:30; the Mac adds today's Angel One session to its intraday history and **retrains both intraday models once** (GitHub's tuned settings, a few minutes, no tuning). The fresher of the Mac's and GitHub's model is used next morning. `MAC_DAILY_TRAINING=0` turns this off | Once a day, after the long-term decision (~17:15) |
