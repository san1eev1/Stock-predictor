...
| **Continuous training** | Models **keep training on historical data in the background**: After each close the **Mac trains once at 16:00** (long-term + both intraday models), then **GitHub trains again at 21:00 and 23:00 IST** on all history since 2005 with self-tuning, replays and profit tuning; the freshest model is always used. **The Mac only predicts during the day** (keeps it cool): live prices, picks and paper trading, plus one short intraday retrain after the close; everything heavy runs on GitHub (`MAC_TRAINING=1` in `.env` turns Mac training back on). |
...
|---|---|---|
| **GitHub Actions** (free) | Long-term model on all history since 2005 + judged paper predictions, then self-tuning for the rest of a ~150-minute run; news relevance model (each tuning round is also checked on paper picks: new settings are kept only if the 5 stocks it would buy beat Nifty at least as often); **long-term historical paper trading** (walk-forward since 2015 with the live rules — the 5 best held — plus a 10-holding comparison, costs included) once a day on the first run after the 15:30 close. Published to the `models` branch. | **Twice each evening, every day**: 21:00 IST (data update, then training) and 23:00 IST, after the Mac's own 16:00 training; started by the Mac on time (`update-market-data.yml` → `train-models.yml`) |
...
| **Mac, after the close** | Live prices stop at 15:30; the Mac adds today's Angel One session to its intraday history and **retrains the long-term model (on its recent history) and both intraday models once** (GitHub's tuned settings, a few minutes, no tuning). The freshest model — the Mac's or GitHub's — is used. `MAC_DAILY_TRAINING=0` turns this off | Once a day, 16:00 |
