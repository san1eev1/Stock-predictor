...
| **Continuous training** | Models **keep training on historical data in the background**: GitHub retrains + self-tunes the long-term and news models on all history since 2005 **twice after each close** (16:45 and 21:00 IST). **The Mac trains nothing** (keeps it cool): it only runs live prices, picks and paper trading and downloads the trained models (`MAC_TRAINING=1` in `.env` turns Mac training back on). |
...
| **Mac, after the close** | Live prices stop at 15:30; the Mac adds today's Angel One session to its intraday history and trains nothing | Every trading day |
